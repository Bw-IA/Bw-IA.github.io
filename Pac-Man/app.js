...
var winLose_texte = document.querySelector('.lives');
var winLosePic = document.getElementById('winLosePic');
winLosePic.style.display = 'none';
...
if(pacman.x > ghost.x ){
if ( map[ghost.y][ghost.x + 1] !== 1){//check if there is a wall
	 ghost.x = ghost.x + 1; //move the ghost closer to pacman to the right
	 drawWorld();
}	
}
if (pacman.x < ghost.x){
if ( map[ghost.y][ghost.x - 1] !== 1){//check if there is a wall
	ghost.x = ghost.x - 1; //move the ghost closer to pacman to the left
	drawWorld()  
	 }
  }
if (pacman.y > ghost.y){
if ( map[ghost.y + 1][ghost.x] !== 1){//check if there is a wall
	ghost.y = ghost.y + 1; //move the ghost closer to pacman down
	drawWorld()  	   
} 
}
if (pacman.y < ghost.y){
if (map[ghost.y - 1][ghost.x] !== 1){//check if there is a wall
	ghost.y = ghost.y - 1; //move the ghost closer to pacman up
	drawWorld()  	
}
}
...
function GameOver() {
   el.style.visibility = 'hidden'; 
    if (lives == 0 ) {
        winLose_texte.innerHTML = "Oh no! No more lives!!! Game Over!!!";
        winLosePic.style.display = 'block';
        winLosePic.src = "images/sadPacman.jpg"
    } else {
        winLose_texte.innerHTML = 'Amazing! You Win !';
        winLosePic.style.display = 'block';
        winLosePic.src = "images/happyPacman.jpg"
    }
    Replay.style.display = 'block'; 
    // Replay.disabled = false; // le bouton 'Replay' est activer et visible
    // Replay.addEventListener('click', NewGame) //ce qui declance une nouvelle fonction Replay qui remet tout a zero
}
